...
$sth = $conn->prepare("SELECT COUNT(*) as nbr  from products where uid=" . $_SESSION['id']);
$sth->execute();

$result = $sth->fetch(PDO::FETCH_ASSOC);
$pnbr = $result['nbr'];

$sth = $conn->prepare("SELECT sum(prix) nbr  from products where uid=" . $_SESSION['id']);
$sth->execute();

$result = $sth->fetch(PDO::FETCH_ASSOC);
$amount = $result['nbr'];

$sth = $conn->prepare("SELECT COUNT(*) nbr  from orders where pid IN (SELECT id from products where uid =" .$_SESSION['id'] ." ) ");
$sth->execute();
